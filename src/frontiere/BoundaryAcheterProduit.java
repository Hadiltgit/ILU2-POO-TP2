		...
		if(controlAcheterProduit.verifierIdentite(nomAcheteur)) {
			               ...
			               if(vendeurs == null  || vendeurs.length == 0) {
			            	  System.out.println("Désolé, personne ne vend ce produit au marché."); 
				               ...
				               int quantiteVolue = Clavier.entrerEntier("Bonjour "+nomAcheteur+" \n Combien de fleurs voulez-vous acheter ?\n");
				               int quantiteAchete = controlAcheterProduit.acheterProduit(nomVendeur, quantiteVolue);
				               
				               if(quantiteAchete == 0) {
				            	   System.out.println(nomAcheteur+" veut acheter "+quantiteVolue+" "+produit+" malheureusement il n’y en a plus !");
				               }else {
				            	   if(quantiteVolue > quantiteAchete) {
				            		   System.out.println(nomAcheteur+" veut acheter "+quantiteVolue+" "+produit+" malheureusement "+nomVendeur+" n’en plus que "+quantiteAchete+".\n");  
